...
;r4 : index of convolution(from 480 to 5)
;r5 : pointer to base of sampled data
;r0 : float of sampled_data AND RESULT OF SUBROUTINES
;r1 : float of filter AND SUB-CONVOLUTION
;R6-R11 : FLOAT OF FILTERS

	LDMIA R0, {R6-R11}
	
;	STMED SP!, {R6-R11}	;store reversed filter
;	MOV R4, SP
	LDR R5, =Sampled_Data ; save the data(480) using Sampled_Data label(32bit, double word).'
	MOV R4, #480
	SUB R4, R4, #1	;GET 479 AT R4, #479 IS NOT EXPRESSED BY THE IMMEDIATE FORM
	
forCONVOLUTION ;CONVOLUTION START
	CMP R4, #5
	BLT forEndCONVOLUTION
	...
	;R12 IS THE RESULT OF CONVOLUTION, AND LET'S CHECK IF IT'S LOWER THAN OR EQUALS -4.7
	LDR R1, FourPointSeven
	BL floatAdd	;INSTEAD CONVOLUTION - (-4.8), PERFORM CONVOLTUTION + 4.7
	CMP R0, #0
	STMEDLE SP!, {R4, R12}	;IF THE CONVOLUTION IS LOWER THAN OR EQUALS -4.8, STORE IT WITH ITS INDEX TO STACK
	SUB R4, R4, #1
	B forCONVOLUTION
forEndCONVOLUTION	;FINISH GETTING AND STORING CONVOLUTIONS
	
	;R6 : CURRENT INDEX AT STACK(TO GET ONLY DATA OR INDEX)
	;R7 : RESULT MEMORY AREA POINTER
	;R0 : DATA OR INDEX THAT WILL BE SOON STORED TO THE RESULT AREA OF MEMORY
	LDR R7, Save_Result_Address;
	MOV R6, SP
	ADD R6, R6, #8	;TO GET THE DATA'S POINTER
storeDATA
	LDR R0, [R6]
	STR R0, [R7], #4
	ADDS R6, R6, #8
	BCC storeDATA
	;data storing finished and start index storing
	MOV R6, SP
	ADD R6, R6, #4 ;TO GET THE INDEX'S POINTER
storeINDEX
	LDR R0, [R6]
	STRH R0, [R7], #2
	ADDS R6, R6, #8
	...
	MOV SP, #0xFFFFFFFC	;FLUSH STACK
	
	mov pc, #0 ; finish

FourPointSeven DCD 0x40966666

floatMul
;R0 : input 1 and return value
;R1 : input 2
;R2 : Aux of input 1
;R3 : Aux of input 2
;R4 : Aux of res
;R5 : Aux of subroutine
	STMED SP!, {R4, R5}	;store  register that will be used
;check exceptoin : case R0 or R1 is 0
	MOV R5, #0x7FFFFFFF
	TST R0, R5
	BEQ endFMul
	TST R1, R5
	MOVEQ R0, R1
	BEQ endFMul
;get multiplied mantissa at R4
	BIC R2, R0, #0xFF000000
	BIC R3, R1, #0xFF000000
	...
	LSLNE R4, R4, #8
	ORRNE R4, R5, LSR #24
	BIC R4, R4, #0x00800000
;get sum of exponenet
	LDR R5, exponentMask
	...
	LSR R3, R3, #23
	ADD R5, R2, R3
	ADDNE R5, R5, #1	;ADD MENTISSA MULTIPLICATION CARRY
	SUB R5, R5, #127
	ORR R4, R4, R5, LSL #23	
;check exception : case exponent of result is over 255 or under 0
	CMP R5, #0
	MOVLT R4, #0
	BLT endFMul
	CMP R5, #255
	LDRGT R4, exponentMask	;store r4 0x7f800000 that means inifinity
;get sign number
	...
	MOV R0, R4
endFMul
	LDMED SP!, {R4, R5}	;load rgister stored
	MOV PC, LR	;return value
	
floatAdd
;R0 : input num1 and return
;R1 : input num2
;R2 : TEMPORARY FOR R0
;R3 : TEMPORARY FOR R1
;R6 : FOR EXPONENT MASK
	STMED SP!, {R6}
	LDR R6, exponentMask
;GET EXPONENT
	AND R2, R0, R6
	AND R3, R1, R6
;ALIGN R0 AND R1 BY EXPONENT, R0 AS BIGGER ONE
	CMP R2, R3
	MOVLO R3, R1
	MOVLO R1, R0
	MOVLO R0, R3
;GET DIFFLENCE OF EXPONENTS R0.EXPONENT - R1.EXPONENT
	AND R2, R0, R6
	MVN R3, R2	;EXCEPTION CHECK
	TST R3, R6
	BEQ infinityException	;EXCEPTION: R0 HAS INFINITY OR NAN NUMBER
	AND R3, R1, R6
	SUB R2, R2, R3
	LSR R2, R2, #23	;SHIFT EXPONENT FROM [30:23] TO [7:0]
	CMP R2, #23	;EXCEPTION CHECK
	LDMEDGT SP!, {R6}
	MOVGT PC, LR	;EXCEPTION : THE SAMLL NUMBER IS SO SMALL THAT ADDITION IS NOT VALID
;GET MANTISSA (SHIFTED BY EXPONENT)
	STMED SP!, {R4-R5}	;TO USE NON PRESERVED REGISTER, STORE THEM TO MEMORY
	;R4:SIGN BIT FOR R0, R5: SIGN BIT FOR R1
	LSL R3, R1, #8
...
;ADDITION OF MANTISSA
	...
	TEQ R4, R5	;CHECK IF IT'S ++,-- OR +-,-+
	AND R5, R0, R6	;GET EXPONENTIAL OF R0
	BNE subcase 
	;IF TWO SIGN BITS IS SAME
	ADDS R2, R2, R3
	LSRCS R2, R2, #1	;IF CARRY SET, CONCATNATE CARRY TO MSB OF RESULT OF SUM AND REGULATE EXPONENT
	ORRCS R2, R2, #0x80000000
	ADDCS R5, R5, #0x00800000
	CMP R5, R6	;CHECK EXCEPTION : EXPONENT OVERFLOW, THE CASE EXPONENT GET 1111_1111, IT MEANS INFINITY
	MOVEQ R2, #0	;SET MANTISSA 0
	B makeResult
subcase
	CMP R4, #0x80000000	;CHECK WHICH REGISTER IS NEGATIVE NUMBER
	ADD R2, R2, #1	;TO AVOID SUBSEQ EXCUTION WHEN R2-R3 EQUALS 0, ADD 1 AT R2; IT'S ENSURED THAT R2[7:0] IS 0
	SUBSNE R2, R2, R3
	SUB R2, R2, #1	;TO AVOID GETTING INCORRECT RESULT WHEN SUBSEQ EXCUTING, SUBSTRACT 1 FROM R2
	SUBSEQ R2, R3, R2
	MOVCS R4, #0	;IF CARRY SET, MARK R4 THAT THE RESULT IS POSITIVE NUMBER
	;IF SUBSTRACTION'S RESULT IS NEGATIVE, MAKE IT POSITIVE NUMBER
	MVNCC R2, R2
	ADDCC R2, R2, #1
	MOVCC R4, #0x80000000	;MARK R4 THAT THE RESULT IS NEGATIVE NUMBER
	;CHECK IF IT'S 0
	TEQ R2, #0
	MOVEQ R5, #0	;EXCEPTION: RESULT IS 0, EXPONENT = 0
	BEQ makeResult	;QUIT
removeZero	;REMOVE 0 ON MSB
	TST R2, #0x80000000
	LSLEQ R2, R2, #1
	SUBEQ R5, R5, #0x00800000
	BEQ removeZero
	TST R5, #0x80000000	;CHECK EXPONENT UNDERFLOW
	MOVNE R2, #0	;EXCEPTION: UNDERFLOW; RESULT IS TOO SMALL LIKE 0, MANTISSA = 0 & EXPONENT = 0
	MOVNE R5, #0
makeResult
;R2:MANTISSA ,R4:SIGN, R5:EXPONENT
	ORR R0, R4, R5
	LSL R2, R2, #1	;REMOVE CHARACTERISTIC
	LSR R2, R2, #9	;SHIFT TO SET MANTISSA CORRECT POSITION
	ORR R0, R0, R2
	LDMED SP!, {R4-R6}
	MOV PC, LR	;QUIT SUBROUTINE
infinityException
	ORR R6, #0x80000000	;CHECK R0'S FRACTION IS 0
	MVN R6, R6
	TST R0, R6
	BNE endException	;IF FRACTION IS NOT 0, RETURN R0
	ADDS R2, R0, R1		;IF FRACTION IS 0, CHECK R1'S FRACTION
	BCC endException
	CMP R2, #0x7F000000	;IF THE RESULT OF SUM IS CARRY SET AND 0x7F00_0000, IT MEANS INFINITY - INFINITY SO RETURN NaN
	MOVEQ R0, #0XFFFFFFFF	;RETURN NAN
endException
	LDMED SP!, {R6}
	MOV PC, LR	;EXCEPTION : INFINITY OR NAN, RETURN WITH THAT NUMBER

exponentMask DCD 0x7F800000
...
fliter	; [1.95, 1.72, -0.431, -1.278, -0.8022, -0.2115]
	DCD 0x3FF9999A, 0x3FDC28F6, 0xBEDCAC08, 0xBFA39581, 0xBF4D5CFB, 0xBE589375
